...
async def list_signals(
    ...
    docs = await db.fraud_signals.find(query).sort([("detected_at", -1)]).limit(limit).to_list(limit)
...
async def list_rules(_: OperatorDep) -> dict:
    ...
    return {
        ...
        "rules": [
            ...
            {
                "rule_id": "HIGH_FREQUENCY_CLAIM",
                "weight": 25,
                "severity": "high",
                "limit": fraud_rules.HIGH_FREQUENCY_LIMIT,
                "days": fraud_rules.HIGH_FREQUENCY_DAYS,
            },
            {
                "rule_id": "SHARED_DEVICE",
                "weight": 20,
                "severity": "medium",
                "limit": fraud_rules.SHARED_DEVICE_LIMIT,
                "days": fraud_rules.SHARED_DEVICE_DAYS,
            },
            {"rule_id": "PESERTA_MENUNGGAK", "weight": 15, "severity": "medium"},
            {
                "rule_id": "REPEATED_FAILED_ATTEMPTS",
                "weight": 15,
                "severity": "medium",
                "limit": fraud_rules.FAILED_ATTEMPT_LIMIT,
                "hours": fraud_rules.FAILED_ATTEMPT_HOURS,
            },

...
def _maxmem_for(n: int, r: int, p: int) -> int:
    """scrypt needs 128*n*r bytes, and OpenSSL refuses anything over a 32 MB default - so n=2**15, r=8 (33.5
    MB) fails with "memory limit exceeded" unless maxmem is raised explicitly.
    """
...
def hash_password(password: str) -> str:
    """scrypt with a random per-password salt. Returns a self-describing string so the cost parameters can be
...
def verify_password(password: str, stored: str) -> bool:
    """Constant-time comparison. Returns False rather than raising on a malformed hash, so a corrupt record
    denies login instead of 500-ing.
    """
    try:
